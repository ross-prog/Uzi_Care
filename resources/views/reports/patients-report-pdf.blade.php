...
<head>
    <meta charset="UTF-8">
    <title>Patients Report - {{ $monthName }}</title>
    <style>
...
<body>
    ...
    <div class="meta-info">
        ...
        <div><strong>Total Records:</strong> {{ $data['totalPatients'] }}</div>
    </div>
    ...
    <div class="summary-section">
        ...
        <div class="summary-grid">
            <div class="summary-card">
                <span class="summary-number">{{ $data['totalPatients'] }}</span>
                <div class="summary-label">Total Patients</div>
            </div>
            <div class="summary-card">
                <span class="summary-number">{{ count($data['departments']) }}</span>
                <div class="summary-label">Departments</div>
            </div>
            <div class="summary-card">
                <span class="summary-number">{{ $data['patients']->sum(function($p) { return $p->nurseNotes->count(); }) }}</span>
    ...
    <table class="table" style="margin-bottom: 15px;">
        ...
        <tbody>
            @foreach($data['departments'] as $department => $count)
                <tr>
                    <td>{{ $department ?: 'Unknown' }}</td>
                    <td>{{ $count }}</td>
                    <td>{{ $data['totalPatients'] > 0 ? number_format(($count / $data['totalPatients']) * 100, 1) : 0 }}%</td>
                </tr>
            @endforeach
        </tbody>
    </table>
        ...
        <table class="table">
            ...
            <tbody>
                @foreach($data['patients'] as $patient)
                    <tr>
                        <td class="patient-id">{{ $patient->student_employee_id }}</td>
                        <td>{{ $patient->full_name ?: ($patient->first_name . ' ' . $patient->last_name) }}</td>
                        <td>{{ $patient->age }}/{{ $patient->sex }}</td>
                        <td>
                            @if($patient->department)
                                <span class="department">{{ Str::limit($patient->department, 20) }}</span>
                            @else
                                -
                            @endif
                        </td>
                        <td class="date-cell">{{ $patient->consultation_date_time ? $patient->consultation_date_time->format('M j, Y H:i') : '-' }}</td>
                        <td>{{ Str::limit($patient->chief_complaints, 50) ?: '-' }}</td>
                        <td>{{ Str::limit($patient->assessment_diagnosis, 40) ?: '-' }}</td>
                        <td>{{ Str::limit($patient->nurse_on_duty, 30) ?: '-' }}</td>
                        <td>
                            @if($patient->nurseNotes && $patient->nurseNotes->count() > 0)
                                {{ $patient->nurseNotes->count() }} note(s)
                                @foreach($patient->nurseNotes->take(1) as $note)
                                    <div class="nurse-notes">
                                        {{ Str::limit($note->nurse_notes, 50) }}
                                    </div>
                                @endforeach
                            @else
                                -
